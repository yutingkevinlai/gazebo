...
void EvaluationPlatform::Load(physics::WorldPtr _world, sdf::ElementPtr /*_sdf*/)
{
	...
	m_test_publisher_ptr = m_node_ptr->Advertise<gazebo::msgs::Request>( "~/evaluation_platform/GGininder" );
	...
	m_subscriber_ptr = m_node_ptr->Subscribe( 	"~/pose_estimation/estimate_result",
												&EvaluationPlatform::_receiveResult,
												this );

	// subscribe to the ended signal of PoseEstimation
	m_ended_subscriber_ptr = m_node_ptr->Subscribe( "~/pose_estimation/estimation_ended",
													&EvaluationPlatform::_receiveEnded,
													this );
...
void EvaluationPlatform::DummyUpdate( const common::UpdateInfo & /*_info*/ )
{
	...
	if(dummyupdate==1)
	{
		...
		//cout << COUT_PREFIX << "\033[7;32m in DummyUpdate \033[m" << endl;
...
//					physics::ModelPtr cur_model = m_world->GetModel( m_models_name[ i ] );
		...
		for(int i=0;i<m_models_name.size();i++) //需要被固定的只有那些只有socket_
		{
			//if(m_world->GetModel(i)->GetName()=="result_visualize_socket")
			//	continue;
			//cout<<COUT_PREFIX<<m_world->GetModel(i)->GetName()<<endl;
			if( m_estimated_models[ i ] )//不要鎖住被估測到的到的工件
			{
				//cout<<"socket_"<<i<<"skipped"<<endl;
				continue;
			}
			//cout<<m_world->GetModel(m_models_name[ i ])->GetName()<<endl;
			m_world->GetModel(m_models_name[ i ])->SetStatic(true);
			m_world->GetModel(m_models_name[ i ])->SetEnabled(false);
...
//		}
	...
	else if(dummyupdate==0)
	{
		for(int i=6;i<m_world->GetModelCount();i++) // 編號6之前是我的手臂 平台 之類的東西
		{
			m_world->GetModel(i)->SetStatic(false);
			m_world->GetModel(i)->SetEnabled(true);
		}

		cout << COUT_PREFIX << "\033[7;32m return to EvaluationPlatform::_onUpdate \033[m" << endl;
//		for( unsigned int i = 0; i < m_models_name.size(); i++ )
//		{
//			physics::ModelPtr cur_model = m_world->GetModel( m_models_name[ i ] );
...
void EvaluationPlatform::_onUpdate( const common::UpdateInfo & /*_info*/ )
{
	...
	if( m_world->GetSimTime().Double() - cur_time > m_check_steady_interval )
	{
		...
		// *************************************************** //
		steady_count++;
		std::cout << "steady_count\tm_consecutive_steady_threshold" << std::endl;
		std::cout << steady_count<<"\t"<<m_consecutive_steady_threshold << std::endl;
		...
		if( steady_count >= m_consecutive_steady_threshold )
		{
			...
			//各model pose輸出到檔案 //
			//*********************//
//			cout << "\033[0;32;34msave workpieces' pose \033[0m" << endl;
...
//			{
//				physics::ModelPtr cur_model = m_world->GetModel( m_models_name[ i ] );
//				rot=cur_model->GetWorldPose().rot.GetAsMatrix3();
//				pos=cur_model->GetWorldPose().pos;
//				pos.x/=6;
//				pos.y/=6;
//				pos.z=0.8;
//
//
//				fout<<cur_model->GetName()<<endl;
//				fout<<rot.m[0][0]<<"\t"<<rot.m[0][1]<<"\t"<<rot.m[0][2]<<"\t"<<pos.x<<endl;
//				fout<<rot.m[1][0]<<"\t"<<rot.m[1][1]<<"\t"<<rot.m[1][2]<<"\t"<<pos.y<<endl;
//				fout<<rot.m[2][0]<<"\t"<<rot.m[2][1]<<"\t"<<rot.m[2][2]<<"\t"<<pos.z<<endl;
//				fout<<"0\t0\t0\t1\t"<<endl;
//
...
//				if(i<10)//要幫補0
//				{
//					m_world->GetModel("billboard0"+int_to_string.str())->SetWorldPose(math::Pose(pos,math::Quaternion(1,0,0,0)));
//					m_world->GetModel("billboard0"+int_to_string.str())->SetGravityMode( false );
//					m_world->GetModel("billboard0"+int_to_string.str())->SetStatic( true );
//
//				}
//				else
//				{
//					m_world->GetModel("billboard"+int_to_string.str())->SetWorldPose(math::Pose(pos,math::Quaternion(1,0,0,0)));
//					m_world->GetModel("billboard"+int_to_string.str())->SetGravityMode( false );
//					m_world->GetModel("billboard"+int_to_string.str())->SetStatic( true );
//				}
//			}
//			fout.close();
...
//
			...
			cout << COUT_PREFIX << "\033[1;35;45m exit onupdate! \033[m" << endl;
			event::Events::DisconnectWorldUpdateBegin( m_update_connection );
			this->m_update_connection = event::Events::ConnectWorldUpdateBegin( boost::bind(&EvaluationPlatform::DummyUpdate, this, _1 ) );
			dummyupdate=1;
...
void EvaluationPlatform::_receiveResult( ConstMsgsPoseEstimationResultPtr &_msg )
{
	...
	for( unsigned int i = 0; i < m_models_name.size(); i++ )
		{
			...
			//原先被鎖住的model，如果是cur_nearest_object的話 可以進入這個判斷式後解鎖
			else if(m_models_name[ i ].compare(0,m_models_name[ i ].size(),cur_nearest_object->GetName())==0)
			{
				physics::ModelPtr cur_model = m_world->GetModel( m_models_name[ i ] );
				cur_model->SetEnabled( false );
				cur_model->SetStatic( true );
	...
	// calculate error
	// 這裡的inverse應該是要將result world的座標原點移到 nearest_model_pose_matrix4
	// error_matrix 的長度就直接是誤差了
	// calculate error
	...
	if( recognized_idx < 0 )
	{
		cerr << CERR_PREFIX << "can't identify recognized object!" << endl;
		exit( -1 );
	}
	...
	if( cur_nearest_object->GetName().compare( 0, m_target_model_names[ recognized_idx ].size(), m_target_model_names[ recognized_idx ] ) == 0 )
	{
		...
		if( translate_error < criteria.translation_threshold )
		{
			...
			cout << COUT_PREFIX << "translation_threshold: " << criteria.translation_threshold << endl;
			cout << COUT_PREFIX << "quaternion_degree_threshold" << criteria.quaternion_degree_threshold << endl;
...
/*	while( !m_evaluation_result_publisher_ptr->HasConnections() )
	...
	if( estimate_correct )
	{
		...
		//送msg給 arm control 告知最近的model
		msgs::Request GGininder;
		GGininder.set_id( 0 );
		GGininder.set_request( cur_nearest_object->GetName() );
		m_test_publisher_ptr->Publish( GGininder );
		cout << COUT_PREFIX << "\033[1;31m" << "send to Arm_Control" << "\033[0m" << endl;}
...
void EvaluationPlatform::_receiveEnded( ConstMsgsRequestPtr &_msg )
{
	...
	//this->m_update_connection = event::Events::ConnectWorldUpdateBegin( boost::bind(&EvaluationPlatform::_onUpdate, this, _1 ) );
	dummyupdate=0;
	cout << COUT_PREFIX << "\033[1;35;45m receive END! \033[m" << endl;
}
...
void EvaluationPlatform::_environmentConstruction()
{
	...
	m_stacking_center =	math::Vector3( 0, 0, m_box_size.z + m_box_wall_thickness + m_throwing_height ) +
						math::Vector3( m_box_center.x, m_box_center.y, 0 );
